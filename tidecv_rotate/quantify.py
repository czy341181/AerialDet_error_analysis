...
from .DOTA_devkit.polyiou import iou_poly
...
def poly_iou(preds, gts):
    iou = np.random.random((len(preds), len(gts)))

    for i, pred in enumerate(preds):
        for j, gt in enumerate(gts):
            iou[i, j] = iou_poly(pred, gt)
...
class TIDEExample:
    ...
    def _run(self):
        ...
        # IoU is [len(detections), len(gt)]  numpy.ndarray
        self.gt_iou = poly_iou(detections, [x[det_type] for x in gt])
...
class TIDERun:
    ...
    def _run(self):
        ...
        # mmcv.dump(self.pred_error, "error.box.json")
    ...
    def _eval_image(self, preds: list, gt: list):
        ...
        if len(preds) == 0:
            ...
            for truth in gt:
                truth_error = {}
                truth_error['flag'] = 'gt'
                truth_error['score'] = truth['score']
                truth_error['bbox'] = truth['bbox']
                truth_error['image'] = truth['image']
                truth_error['class'] = truth['class']
                # if not truth['ignore']:
                ...
                if self.run_errors:
                    ...
                    truth_error['error'] = 'mis'
                    self.pred_error.append(truth_error)
            return
        ...
        for pred_idx, pred in enumerate(preds):
            ...
            pred_error = {}
            pred_error['flag'] = 'pred'
            pred_error['score'] = pred['score']
            pred_error['bbox'] = pred['bbox']
            pred_error['image'] = pred['image']
            pred_error['class'] = pred['class']
            ...
            if self.run_errors and (pred['used'] == False or pred['used'] == None):
                ...
                pred_error['error'] = 'both'

                self.pred_error.append(pred_error)

            else:
                pred_error['error'] = 'true'
                self.pred_error.append(pred_error)
